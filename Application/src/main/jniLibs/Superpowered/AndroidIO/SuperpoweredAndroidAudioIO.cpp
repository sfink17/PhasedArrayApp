...
static void SuperpoweredAndroidAudioIO_OutputCallback(SLAndroidSimpleBufferQueueItf caller, void *pContext) {
    ...
    if (output) {
        if (internals->readBufferIndex < internals->numBuffers - 1){
            ...
            Java_com_example_android_bluetoothchat_PhasedArrayFragment_bufferRead(JNIEnv *env, jobject obj) {
                jclass cls = (*env)->GetObjectClass(env, obj);
                jmethodID mid = (*env)->GetMethodID(cls, "readTime", "()V");
                if (mid == 0) return;
...
void SuperpoweredAndroidAudioIO::callback() {
    JNIEnv * env;
